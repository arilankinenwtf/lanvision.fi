...
<div class="ccm-faq-block-container">
    <div class="form-group">
        <label class="control-label" for="headingType"><?=t('Heading type')?></label>
        <select class="form-control" id="headingType" name="headingType" value="<?php echo $headingType?>">
            <option value="h2" <?php if($headingType == 'h2') echo 'selected';?>>H2</option>
            <option value="h3"<?php if($headingType == 'h3') echo 'selected';?>>H3</option>
            <option value="h4"<?php if($headingType == 'h4') echo 'selected';?>>H4</option>
            <option value="h5"<?php if($headingType == 'h5') echo 'selected';?>>H5</option>
        </select>
    </div>
    ...
    if ($rows) {
        foreach ($rows as $row) { ?>
            <div class="ccm-faq-entry well entry-closed">
                <p class="entry-collapse-text"><?php echo $row['title'] ? $row['title'] : ''; ?></p>
...
$(document).ready(function() {
    ...
    $('.ccm-add-faq-entry', container).click(function() {
        ...
        $('.ccm-faq-entry').not('.entry-closed').each(function() {
            ...
            var linkTitleText = $(this).find('input[name="title[]"]').val();
            if (linkTitleText) {
    ...
    $(container).on('click','.ccm-edit-entry', function() {
        ...
        if (thisEditButton.data('entryEditText') === thisEditButton.text()) {
            ...
            var linkTitleText = closestEntry.find('input[name="title[]"]').val();
            if (linkTitleText) {
